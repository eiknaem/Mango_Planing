...
const service = {
  ...
  async getServer(url) {
    ...
    let error = null;
    // console.log("axioscustom2.get(url)", axioscustom2.get(url));
  ...
  async postServerJson(url, data) {
    const site_ = await getDataStorage("sitevalue_key");
    const token_ = await getDataStorage("token_key");
    ...
    const resp = await axioscustom2.post(url, data, {
